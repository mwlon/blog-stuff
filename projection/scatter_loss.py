...
def add_traditional():
  ...
  sph = lattice.sph
  euc = lattice.euc
  triangles = traditional.filter_triangles(
    lattice.triangles, traditional.calc_filter(sph, args.compare_latitude)
  )
...
def add_trained():
  for name in filter_trained(args.filter):
    ...
    euc = calc_euc(sph)
    triangles = traditional.filter_triangles(
      loaded.triangles, traditional.calc_filter(sph, args.compare_latitude)
    )
    triples = triples_for_triangles(triangles)
...
offsets = {
  ...
  "Natural Earth": (0.45, 0.33),
  "Equirectangular": (0.3, 0),
  "Martin I": (-0.03, -0.1),
  "Eckert IV": (0.2, 0.0),
}
colors = []
...
ax.axhline(
  2, lw=2, linestyle="dashed", zorder=-100000, label="conformal", color="#5555ee"
)
ax.axvline(
  2, lw=2, linestyle="dashed", zorder=-100000, label="equal area", color="#ff0077"
)
...
plt.tight_layout()
plt.savefig("results/loss_scatter.svg")
plt.show()
